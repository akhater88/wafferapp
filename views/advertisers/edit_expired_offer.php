...
<script type="text/javascript"> 
		...
		$('.Paginate').click(function(){
			...
			$.post('<?php echo __LINK_PATH;?>advertisers/edit_expired_offer/AJAX/Y/',{Page:Page},function(data){
				$('#Container').html(data);
		...
		jQuery.leave = function(){
			$.post('<?php echo __LINK_PATH;?>advertisers/edit_expired_offer/AJAX/Y/',function(data){
					$('#Container').html(data);
				})
			};	
		...
		$('.Del_Btn_Main').click(function(){
			...
			$.post('<?php echo __LINK_PATH;?>advertisers/delete_expired_selected_offer/AJAX/Y/',{ID:ID},function(data){
				$('#Container').html(data);
				});
			}
			});	
...
foreach($results as $rows)
	...
	<tr>
		...
		<td>
		<div align="center">
		<img style="cursor:pointer " src="<?php echo __SCRIPT_PATH;?>images/edit.gif" width="30" height="30" onClick="$.editData('تـعـديـل الـعـرض','<?php echo __LINK_PATH;?>advertisers/edit_selected_expired_offer/Member/<?php echo $ID;?>/AJAX/Y/',950,550)" />
		</div>
		</td>
		<td><div align="center"><?php echo $End_Date;?></div></td>
