...
function initThemeRoller() {	
	//$("#theme_selector").change(function() {
		var theme = 'redmond';//$(this).val();
		//var loader = $("#jquery_ui_theme_loader");
		$('#jquery_ui_theme_loader').attr("href", "http://www.softiletest.com/mymvc_test/mypanel/includes/css/themes/"+theme+"/jquery-ui.css");
		//loader.attr("href", "http://www.softiletest.com/windows/css/themes/"+theme+"/jquery-ui.css");
	//});
}
...
function createWindowWithCallBack(title,url,width,height){
	$.window({
					title: title,
					url: url, // load window_block5 html content
					x: -1,//150,               // the x-axis value on screen, if -1 means put on screen center
					y: -1,//100,               // the y-axis value on screen, if -1 means put on screen center
