...
class cache_modified
{
    ...
    function __construct($Path){
		$this->cacheTime = time() - filemtime($Path);
		
        $this->cacheFile = base64_encode($_SERVER['REQUEST_URI']);
        $this->cacheFileName = $this->cacheDir.'/'.$this->cacheFile.'.html';
    ...
    function start(){
        ...
        if(!in_array($location[0],$this->doNotCache)){
            if(file_exists($this->cacheFileName) && (time() - filemtime($this->cacheFileName)) < $this->cacheTime && $this->cacheLog[$this->cacheFile] == 1){
				...
				$_SESSION['Cache_Needed'] = false;
            }else{
				$_SESSION['Cache_Needed'] = true;
                $this->caching = true;
                ob_start();
            }
        }
