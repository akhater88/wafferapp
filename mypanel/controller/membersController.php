				...
				if($Level == '2')
					{
						...
						elseif(($Contact_Email != NULL)&&(!$Validate_Contact_Email))
						...
						else
							{
								...
								$sql = 'UPDATE users SET First_Name = ?,user_name = ?,Level = ?,Company_Name = ?,Phone_Number = ?,Cell_Phone = ?,Sender_Name = ?,Address = ?,Starts_Date = ?,End_Date = ?,Ads_Cat = ?,Country = ?,Contact_Email = ? WHERE ID = ?';
								$Execute_Array = array($First_Name,$user_name,$Level,$Company_Name,$Phone_Number,$Cell_Phone,$Sender_Name,$Address,$Starts_Date,$End_Date,$Ads_Cat,$Country,$Contact_Email,$ID);
				...
				if($Level == '2')
					{
						...
						elseif(($Contact_Email != NULL) && (!$Validate_Contact_Email))
						...
						else
							{
								...
								$Salt = $create_member->getPasswordSalt();
								$Enc_PW = $create_member->getPasswordHash($Salt,$password);
								$sql = 'INSERT INTO users (First_Name,user_name,password,Salt,Level,Company_Name,Phone_Number,Cell_Phone,Sender_Name,Address,Starts_Date,End_Date,Ads_Cat,Country,Contact_Email,Status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
								$Execute_Array = array($First_Name,$user_name,$Enc_PW,$Salt,$Level,$Company_Name,$Phone_Number,$Cell_Phone,$Sender_Name,$Address,$Starts_Date,$End_Date,$Ads_Cat,$Country,$Contact_Email,'1');
