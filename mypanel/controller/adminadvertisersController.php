...
class adminadvertisersController extends baseController {
		...
		public function submit_offer_search_results()
			{
				...
				if($Merchants_ID == '999')
					{
						$sql = 'SELECT ID,AID FROM '.$Table_Offers.' WHERE AID != ? AND Status != ?';
						$Execute_Array = array('ABC','0');
					}
				else
					{
						$sql = 'SELECT ID,AID FROM '.$Table_Offers.' WHERE AID = ?';
				...
				foreach($results as $rows)
					{
						if($Country_ID != '999')
							{
								$sql = 'SELECT Country FROM users WHERE ID = ? AND Status = ?';
								$Execute_Array = array($rows->AID,'1');
								$Country = $Display->Display_Single_Info_Modified($sql,'Country',$Execute_Array);
				...
				//Filter offer ID based on city selection:
				...
				if(count($Offers_ID_Array))
					{
						...
						$this->registry->template->show('adminadvertisers/submit_offer_search_results');
		...
		public function submit_offer_search_results_2()
			{
				$Display = new sql();
				$Today = date('d-m-Y');
				$Status = $_POST['Status'];
				...
				//Search offers first:
				$OID_Array_City = array();
				$OID_Array_Creation = array();
				$OID_Start_End_Dates = array();
				$OID_Status_Array = array();
				$OID_From_Offers_Array = array();
				
				$AID_Country_Array = array();
				$AID_ID_Array = array();
				$AID_Cat_Array = array();
				$AID_Services_Array = array();
				...
				if((($Creation_Start_Date == $Today)||($Creation_End_Date == $Today)))
					{
						$sql = 'SELECT RID FROM logs WHERE Table_Name = ? AND Action = ? ';
						$Execute_Array = array($Table_Offers,'1');
						$results = $Display->Display_Info($sql,$Execute_Array);
						foreach($results as $rows)
							{
								$OID_Array_Creation[] =  $rows->RID;
							}
					}
				else
					{
						$Creation_Start_Date = date('Y-m-d G:i:s',strtotime($Creation_Start_Date));
						$Creation_End_Date = date('Y-m-d G:i:s',strtotime($Creation_End_Date));
						$sql = 'SELECT RID FROM logs WHERE Table_Name = ? AND Action = ? AND (Time_Stamp BETWEEN ? AND ? )';
						$Execute_Array = array($Table_Offers,'1',$Creation_Start_Date,$Creation_End_Date);
						$results = $Display->Display_Info($sql,$Execute_Array);
						foreach($results as $rows)
							{
								$OID_Array_Creation[] =  $rows->RID;
				...
				//Search based on start and end dates:
				if((($Start_Date == $Today)&&($End_Date == $Today)))
					{
						$sql = 'SELECT ID FROM '.$Table_Offers.' WHERE Status != ?';
						$Execute_Array = array('999');
						$results = $Display->Display_Info($sql,$Execute_Array);
						foreach($results as $rows)
							{
								$OID_Start_End_Dates[] =  $rows->ID;
							}
					}
				else
					{
						...
						$sql = 'SELECT ID,Start_Date,End_Date FROM '.$Table_Offers.' WHERE Status != ? ';
						$Execute_Array = array('999');
						$results = $Display->Display_Info($sql,$Execute_Array);
						foreach($results as $rows)
							{
								$DB_Start_Date = strtotime($rows->Start_Date);
								$DB_End_Date = strtotime($rows->End_Date);
								if(($DB_Start_Date >= $Start_Date) && ($DB_End_Date <= $End_Date))
									{
										$OID_Start_End_Dates[] =  $rows->ID;
									}
							}
				...
				if($Country_ID == '999')
					{
						$sql = 'SELECT ID FROM users WHERE Level = ? AND Status = ? ';
						$Execute_Array = array('2','1');
						$results = $Display->Display_Info($sql,$Execute_Array);
						foreach($results as $rows)
							{
								$AID_Country_Array[] =  $rows->ID;
							}
					}
				else
					{
				...
				if($Merchants_ID == '999')
					{
						...
						foreach($results as $rows)
							{
								$AID_ID_Array[] =  $rows->ID;
							}
					}
				else
					{
						$AID_ID_Array[] = $Merchants_ID;
					}
				//Seach based on cat ID:
				if($Category_ID == '999')
					{
						$sql = 'SELECT ID FROM users WHERE Level = ? AND Status = ? ';
						$Execute_Array = array('2','1');
						...
						foreach($results as $rows)
							{
								$AID_Cat_Array[] =  $rows->ID;
							}
					}
				else
					{
						$sql = 'SELECT ID FROM users WHERE Level = ? AND Status = ? AND Ads_Cat = ?';
						$Execute_Array = array('2','1',$Category_ID);
						$results = $Display->Display_Info($sql,$Execute_Array);
						foreach($results as $rows)
							{
								$AID_Cat_Array[] =  $rows->ID;
				...
				if($Services_ID = '999')
					{
						...
						$Execute_Array = array($Services_ID,'1');
						$results = $Display->Display_Info($sql,$Execute_Array);
						foreach($results as $rows)
							{
								$AID_Services_Array[] =  $rows->MID;
							}
					}
				else
					{
				...
				$result_aid_filtered = array_intersect($AID_Country_Array,$AID_ID_Array,$AID_Cat_Array,$AID_Services_Array);
				
				$result_oid_filtered = array_intersect($OID_Status_Array,$OID_Array_Creation,$OID_Start_End_Dates);
				
				foreach($result_aid_filtered as $value)
					{
						$sql = 'SELECT ID FROM '.$Table_Offers.' WHERE AID = ?';
						$Execute_Array = array($value);
						$results = $Display->Display_Info($sql,$Execute_Array);
						foreach($results as $rows)
							{
								$OID_From_Offers_Array[] =  $rows->ID;
							}
					}
				
				$OID_Array_Creation = array();
				$MyOID = array();
				$result_final = array_intersect($OID_From_Offers_Array,$result_oid_filtered);
				echo 'AID filtered :'.count($result_aid_filtered).'<BR />';
				echo 'OID filtered :'.count($result_oid_filtered).'<BR />';
				echo 'Final filteration : '.count($OID_From_Offers_Array).'<BR />';
				if((($Creation_Start_Date == $Today)||($Creation_End_Date == $Today)))
					{
						$result_final = array_intersect($OID_From_Offers_Array,$result_oid_filtered);
						foreach($result_final as $value)
							{
								$MyOID[] = $value;
							}
					}
				else
					{
						...
						$result_final_2 = array_intersect($OID_Array_Creation,$result_final);
						foreach($result_final_2 as $value)
							{
								$MyOID[] = $value;
							}
					}
				...
				$pagenum  = @$_POST['Page'];
				$paginate = new paginate_array($MyOID,'10',$pagenum);
				$Count = $paginate->Records();
				
				$this->registry->template->Page = $pagenum;
				$this->registry->template->Count = $Count;
				$this->registry->template->Last = $paginate->Calculate_Last($Count);
				$MyOID = $paginate->Paginate();
				...
				foreach($MyOID as $value)
					{
						...
						foreach($results as $rows)
							{
								$Sender_Name_ID[] = $rows->AID;
								$Start_Date_Array[] = date('d-m-Y',strtotime($rows->Start_Date));
								$End_Date_Array[] = date('d-m-Y',strtotime($rows->End_Date));
								$Status_Int = $rows->Status;
								switch($Status_Int)
									{
										case '0':
										$Status_Array[] = 'مـمـسـوح';
								...
								$sql_user = 'SELECT Sender_Name,Country FROM users WHERE ID = ?';
								$Execute_Array = array($rows->AID);
								$results_user = $Display->Display_Info($sql_user,$Execute_Array);
								foreach($results_user as $rows_users)
									{
										$Sender_Name_Array[] = $rows_users->Sender_Name;
										$sql_country = 'SELECT Name FROM '.$Table_Country.' WHERE ID = ?';
										$Execute_Array = array($rows_users->Country);
										$Country_Names[] = $Display->Display_Single_Info_Modified($sql_country,'Name',$Execute_Array);
									}
							}
					}
				$this->registry->template->MyOID = $MyOID;
				$this->registry->template->Start_Date_Array = $Start_Date_Array;
				$this->registry->template->End_Date_Array = $End_Date_Array;
				$this->registry->template->Status_Array = $Status_Array;
				$this->registry->template->Sender_Name_Array = $Sender_Name_Array;
		...
		public function search_my_offer()
			{
				...
				$this->registry->template->Country_Names = $Country_Names;
				$this->registry->template->show('adminadvertisers/search_my_offer');
			}
		public function submit_country_name()
			{
				...
				$this->registry->template->show('adminadvertisers/submit_country_name');
			}
		public function submit_city_name()
			{
				...
				/*$sql = 'SELECT Offer_Title FROM '.$Table_Offers.' WHERE Status = ? AND AID = ?';
				$Execute_Array = array('1',$MID);
				$results = $Display->Display_Info($sql,$Execute_Array);
				foreach($results as $rows)
						{
							echo $rows->Offer_Title.'<BR />';
						}*/
				if($MID == '999')
					{
						if($CID == '999')
				...
				$this->registry->template->show('adminadvertisers/submit_city_name');
			}
		public function submit_category_name()
			{
				...
				$this->registry->template->show('adminadvertisers/submit_category_name');
			}
		public function submit_service_name()
			{
				...
				$this->registry->template->show('adminadvertisers/submit_service_name');
			}
		public function submit_status_name()
			{
				...
				$this->registry->template->show('adminadvertisers/submit_status_name');
			}
		...
		public function delete_selected_offer()
			{
				...
				$sql = 'SELECT AID FROM '.$Table.' WHERE ID = ? AND Status = ?';
				...
				$this->show_client_offers_par($AID,$Page);
			}
		...
		private function show_client_offers_par($AID,$Page='')
			{
				$Display = new sql();
				...
				if(count($AID_Array))
					{
						...
						foreach($results as $rows)
							{
								...
								$sql_log = 'SELECT Time_Stamp FROM logs WHERE RID = ? AND Action = ? AND Table_Name = ?';
								$Execute_Array_Log = array($rows->ID,'1',$Table);
								$Creation_Date = $Display->Display_Single_Info_Modified($sql_log,'Time_Stamp',$Execute_Array_Log);
								$Time_Stamp_Array[] = date('d-m-Y G:i:s',strtotime($Creation_Date));
								$sql_users = 'SELECT Sender_Name,Country FROM users WHERE ID = ?';
								$Execute_Array = array($rows->AID);
								$result_users = $Display->Display_Info($sql_users,$Execute_Array);
								foreach($result_users as $rows_users)
									{
										$First_Name_Array[] = $rows_users->Sender_Name;
										$sql_country = 'SELECT Name FROM '.$Table_Country.' WHERE ID = ?';
										$Execute_Array = array($rows_users->Country);
						...
						$this->registry->template->Start_Date_Array = $Start_Date_Array;
						$this->registry->template->Time_Stamp_Array = $Time_Stamp_Array;
						$this->registry->template->show('adminadvertisers/show_pending_offers');
					}
				else
					{
						$this->registry->template->show('admin_search/no_results');
		...
		public function submit_returned_email()
			{
				...
				if($Comments == NULL)
				...
				else
					{
						...
						if($Country == '18')
							{
								$my_mail->Send_SMTP($body,'mail.wafferapp.com','jor@wafferapp.com','FU{t?uMe7XaT','jor@wafferapp.com','Waffer Team','رســالــة مـرجــعــة',$Recipient,$First_Name);
								$my_mail->Send_SMTP($body,'mail.wafferapp.com','jor@wafferapp.com','FU{t?uMe7XaT','jor@wafferapp.com','Waffer Team','رســالــة مـرجــعــة','msgs@wafferapp.com','Waffer Team');
							}
						else
							{
								$my_mail->Send_SMTP($body,'mail.wafferapp.com','ksa@wafferapp.com','&dywIvl4@~]=','ksa@wafferapp.com','Waffer Team','رســالــة مـرجــعــة',$Recipient,$First_Name);
								$my_mail->Send_SMTP($body,'mail.wafferapp.com','ksa@wafferapp.com','&dywIvl4@~]=','ksa@wafferapp.com','Waffer Team','رســالــة مـرجــعــة','msgs@wafferapp.com','Waffer Team');
							}
		...
		public function edit_selected_offer_from_pending()
			{
				...
				$Execute_Array = array($ID);
				$this->registry->template->results = $Display->Display_Info($sql,$Execute_Array);
				$this->registry->template->show('adminadvertisers/edit_selected_offer_from_pending');
			}
		public function pending_offer()
			{
				...
				$this->registry->template->show('adminadvertisers/pending_offer');
			}
		...
		public function submit_edit_offer()
			{
				...
				elseif($Invalid_Date)
				...
				else
					{
						...
						$Execute_Array = array($Ads_Sub_Cat,$Offer_Title,$Offer_Content_Stripped,$Start_Date,$End_Date,$ID);
						$Display->Execute($sql,$Execute_Array);
						$Action_Time = date('Y-m-d G:i:s');
						$Display->create_log($ID,$Table,'2',$Action_Time,$OID,'Edited a voucher');
						$sql = 'DELETE FROM '.$Table_City.' WHERE OID = ?';
