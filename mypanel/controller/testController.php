...
class testController extends baseController {
		...
		public function create_relation()
			{
				...
				$sql = 'SELECT ID,City FROM '.$Table.' WHERE Status = ?';
				$Execute_Array = array('1');
				$results = $Display->Display_Info($sql,$Execute_Array);
				foreach($results as $rows)
					{
						$sql_city = 'INSERT INTO '.$Table_City.' (OID,CID) VALUES (?,?)';
						$Execute_Array = array($rows->ID,$rows->City);
						$Display->Execute($sql_city,$Execute_Array);
					}
			}
		public function trimmer()
			{
				...
				foreach($results as $rows)
					{
						$ID = $rows->ID;
						$Offer_Content = trim(strip_tags($rows->Offer_Content));
						$sql_insert = 'UPDATE '.$Table.' SET Offer_Content = ? WHERE ID = ?';
		...
		public function add_offers()
			{
				...
				$Offer_Content_Stripped = 'تنزيلات عالمية و بأسعــار ممتازة تنزيلات عالمية و بأسعــار ممتازة
										 تنزيلات عالمية و بأسعــار ممتازة
										تنزيلات عالمية و بأسعــار ممتازة
										تنزيلات عالمية و بأسعــار ممتازة
										تنزيلات عالمية و بأسعــار ممتازة
										تنزيلات عالمية و بأسعــار ممتازة
										...
										تنزيلات عالمية و بأسعــار ممتازة
												...
												';
				$Offer_Content_Stripped = trim($Offer_Content_Stripped);
				$OID = $_SESSION['User_ID'];
				for($i = 0;$i<=40 ;$i++)
				{
				$sql = 'INSERT INTO '.$Table.' (AID,Ads_Sub_Cat,Offer_Title,Offer_Content,Start_Date,End_Date,Status) VALUES (?,?,?,?,?,?,?)';
				$Execute_Array = array('52','5','تنزيلات عالمية و بأسعــار ممتازة',$Offer_Content_Stripped,'2012-04-09','2012-08-09','1');
				$Display->Execute($sql,$Execute_Array);
				$RID = $Display->Execute($sql,$Execute_Array,'1',$Table);
				$Action_Time = date('Y-m-d G:i:s');
				$Display->create_log($RID,$Table,'1',$Action_Time,$OID,'added new offers');
				
				$sql = 'INSERT INTO '.$Table_City.' (OID,CID) VALUES (?,?)';
				$Execute_Array = array($RID,'7');
		...
		public function display_ids()
			{
				...
				$sql = 'SELECT * FROM country_english WHERE Status = ?';
				$Execute_Array = array('1');
				$results = $Display->Display_Info($sql,$Execute_Array);
				foreach($results as $rows)
					{
				...
				$sql = 'SELECT * FROM city_english WHERE Status = ?';
				$Execute_Array = array('1');
				$results = $Display->Display_Info($sql,$Execute_Array);
				foreach($results as $rows)
					{
				...
				$sql = 'SELECT * FROM ads_sub_cat_english WHERE Status = ?';
				$Execute_Array = array('1');
				$results = $Display->Display_Info($sql,$Execute_Array);
				foreach($results as $rows)
					{
						echo $rows->ID.' '.$rows->Sub_Cat_Name.'<BR />';
					}
		...
		public function display_messages_by_cat_city()
			{
				...
				$City = '4';
				//$Key = @$_POST['Key'];
				$verify_key_modified = true; //$this->verify_key_modified($Key);
												...
												switch($rows_offers->Ads_Sub_Cat)
													{
														...
														$Offer_Content .= ' رقـم كـوبـون الـخـصـم  '.$Coupon;
		...
		public function send_email_to_admin_mobile()
			{
				$Display = new sql_modified();
				$Country_ID = '18';
				$Email = 'abc@hot.com';
				...
				$sql = 'SELECT ID FROM mobile_users WHERE Country_ID = ?';
				$Execute_Array = array('18'); //Jordan
				$Jordan_Users = $Display->Total_Records($sql,$Execute_Array);
				
				$sql = 'SELECT ID FROM mobile_users WHERE Country_ID = ?';
				$Execute_Array = array('19'); //Jordan
				...
				$my_mail->Send_SMTP($body,'mail.wafferapp.com','jor@wafferapp.com','FU{t?uMe7XaT','jor@wafferapp.com','Waffer Team','Waffer','fadi_adawi@hotmail.com','MR. Hazem');
