...
<script type="text/javascript"> 
	$(document).ready(function() { 
		...
		$('.Paginate').click(function(){
			...
			$.post('<?php echo __LINK_PATH;?>operators/show_pending_offers/AJAX/Y/',{Page:Page,Country:Country},function(data){
				$('#Container').html(data);
		...
		jQuery.leave = function(){
			...
			var Country = '<?php echo $Country_ID;?>';
			$.post('<?php echo __LINK_PATH;?>operators/show_pending_offers/Level/2/AJAX/Y/',{Page:Page,Country:Country},function(data){
					$('#Container').html(data);
				})
			};	
...
if(count($Offer_ID_Array))
	{
		...
		foreach($Offer_ID_Array as $value)
			...
			?>
			<tr>
		  	<td style="padding-top:4px;" class="table-orange2"><span style="cursor:pointer " onClick="$.editData('تـعـديـل مـعـلــومـات الـعـمـيــل','<?php echo __LINK_PATH;?>operators/edit_selected_offer_from_pending/Member/<?php echo $value;?>/AJAX/Y/',950,600)"><img alt="" src="<?php echo __SCRIPT_PATH;?>images/details.jpg" width="80" height="24" class="style2" border="0" /></span></td>
