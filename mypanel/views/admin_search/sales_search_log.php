...
<script type="text/javascript"> 
	$(document).ready(function() {
		$('.Del_Btn_Main').click(function(){
			...
			$.post('<?php echo __LINK_PATH;?>admin_search/delete_selected_sales/AJAX/Y/',{ID:ID,Page:Page,CID:CID},function(data){
				$(Div).remove();
		...
		$('.Paginate').click(function(){
			var Page = $(this).attr('id'); 
			var CID = '<?php echo $CID;?>';
			$.post('<?php echo __LINK_PATH;?>admin_search/sales_search_log/AJAX/Y/',{Page:Page,CID:CID},function(data){
				$('#Container').html(data);
		...
		var CID = '<?php echo $CID;?>';
			$.post('<?php echo __LINK_PATH;?>admin_search/sales_search_log/Level/3/AJAX/Y/',{Page:Page,CID:CID},function(data){
					$('#Container').html(data);
				})
			};	
...
if(count($ID_Array)&& is_array($ID_Array))
	{
		...
		<table style=" width:100%; text-align:center; line-height:30px;" cellpadding="0" cellspacing="0" class="style1" dir="ltr">
		  <tr>
			...
			<td width="13%" class="table-orange" style="height: 40px;">كـلـمــة الـسـر</td>
		...
		foreach($ID_Array as $value)
			{
						...
						<td style="padding-top:4px;" class="table-orange2"><img style="cursor:pointer " src="<?php echo __SCRIPT_PATH;?>images/edit.png" width="20" height="20" onClick="$.editData('تـعـديـل مـعـلـومـات الإداري','<?php echo __LINK_PATH;?>members/edit_selected/Member/<?php echo $ID_Array[$Index];?>/Level/3/AJAX/Y/',950,300)"></td>
