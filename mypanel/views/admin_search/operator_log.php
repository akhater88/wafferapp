...
<script type="text/javascript"> 
	$(document).ready(function() {
		...
		$('.Paginate').click(function(){
			...
			$.post('<?php echo __LINK_PATH;?>admin_search/operator_log/AJAX/Y/',{Page:Page},function(data){
				$('#Container').html(data);
				$('#Product_Container').hide().delay(300).fadeIn();
				});
			});	
		jQuery.leave = function(){
			$.post('<?php echo __LINK_PATH;?>admin_search/display_search_results/Level/4/AJAX/Y/',function(data){
					$('#Container').html(data);
				})
			};	
...
if(count($results)&& is_array($results))
	{
		...
		<table style=" width:100%; text-align:center; line-height:30px;" cellpadding="0" cellspacing="0" class="style1" dir="ltr">
		  <tr>
			<td width="6%" class="table-orange" style="height: 40px;">مـسـح</td>
			<td width="7%" class="table-orange" style="height: 40px;">تـعــديـل</td>
			...
			<td width="14%" class="table-orange" style="height: 40px;">تـاريـخ الإدخــال</td>
			<td width="20%" class="table-orange" style="height: 40px;">الـبـريـد الإلـكـتـرونــي</td>
			<td width="27%" class="table-orange" style="height: 40px;">الإســم</td>
		  </tr>
		<?php
		foreach($results as $rows)
			{
				...
				foreach($results_log as $rows_log)
					...
					?>
						...
						<td style="padding-top:4px;" class="table-orange2"><img style="cursor:pointer " src="<?php echo __SCRIPT_PATH;?>images/edit.png" width="20" height="20" onClick="$.editData('تـعـديـل مـعـلـومـات الإداري','<?php echo __LINK_PATH;?>members/edit_selected/Member/<?php echo $rows_log->RID;?>/Level/4/AJAX/Y/',950,300)"></td>
